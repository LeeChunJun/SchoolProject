...
public class AccountCheck {
	...
	public static boolean checkJWAccount(String id, String password) {
		try {
			...
			if(!retStr.toString().contains("欢迎您")){
				return false;
			}
			
		} catch (IOException e) {
			Log.e(TAG, "验证教务在线账户出错: "+e);
			return false;
		}
		return true;
	}
	
	public static boolean checkTUAccount(String id, String password){
		try {
			String url1 = "http://tsg.jxnu.edu.cn:8080/reader/redr_verify.php";  
	        URL url = new URL(url1);
	        HttpURLConnection connection = (HttpURLConnection) url.openConnection();  
	        connection.setDoOutput(true);
	        connection.setDoInput(true);  
	        connection.setRequestMethod("POST");  
	        connection.setUseCaches(false);  
	        connection.addRequestProperty("Accept", "application/x-ms-application, image/jpeg, application/xaml+xml, image/gif, image/pjpeg, application/x-ms-xbap, application/x-silverlight, */*");  
	        connection.addRequestProperty("Referer", "http://tsg.jxnu.edu.cn:8080/reader/login.php");
	        connection.addRequestProperty("Accept-Language", "zh-CN");
	        connection.addRequestProperty("Content-Type", "application/x-www-form-urlencoded");
	        connection.addRequestProperty("Accept-Encoding", "gzip, deflate");
	        connection.addRequestProperty("Cookie", "_ga=GA1.3.1058935975.1484454967; PHPSESSID=ochomouk2c6cjbksaohja97594");
	        ...
	        br.close();  
	        in.close();
			if(!retStr.toString().contains("注销")){
				return false;
			}
	        
		} catch (IOException e) {
			Log.e(TAG, "验证图书馆账户出错: "+e);
			return false;
		}
		return true;
	}
