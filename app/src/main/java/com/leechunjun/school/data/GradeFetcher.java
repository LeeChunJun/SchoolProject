...
public class GradeFetcher {
	...
	public GradeFetcher(Context context, String id, String password, String time, NetResponse netResponse) {
		super();
	...
	public String setCookies(String id, String password, String time)
		...
		return getGrade(cookies);
	}

	public String getGrade(String cookies) throws IOException {
		String urlPath = "http://jwc.jxnu.edu.cn/MyControl/All_Display.aspx?UserControl=xfz_cj.ascx&Action=Personal";
	...
	public List<Grade> fetchItem(){
		List<Grade> gradeList = new ArrayList<Grade>();
		try {
			String gradeHtmlString = setCookies(id, password, time);
			parseItem(gradeList,gradeHtmlString);
		}catch (Exception e) {
			Log.e(TAG, "获取成绩失败: "+e);
			netResponse.onFinished("获取成绩失败!", -1);
		}
		
		return gradeList;
	}

	private void parseItem(List<Grade> gradeList, String gradeHtmlString) {
		String html=gradeHtmlString.replace("&nbsp;", "") ;
		Document doc = Jsoup.parse(html);
		Element table = doc.getElementById("_ctl0_dgContent");
        ...
        for(int i = 1;i<trs.size();i++){
            ...
            int q = 0;
            grade.setExamTime(currentTerm);
            grade.setCourseNo(c1[q++]);
            grade.setCourseName(c1[q++]);
            grade.setExamPoint(c1[q++]);
            grade.setExamScore(c1[q++]);
            grade.setExamAgainScore(c1[q++]);
            grade.setExamStandardScore(c1[q++]);
            gradeList.add(grade);
        }
        
        netResponse.onFinished("获取成绩成功", 0);
	}
